...
class FFTLoss(nn.Module):
    def __init__(self, loss_weight_fft=0.01, reduction='mean'):
        super(FFTLoss, self).__init__()
        self.loss_weight_fft = loss_weight_fft
    ...
    def forward(self, pred, target):
        ...
        pred_y_imag = pred_fft.imag
        pred_y_real = pred_fft.real
        pred_fft_output = torch.complex(pred_y_real, pred_y_imag)

        # HR fft
        hr_fft = torch.fft.rfft2(target, norm='backward') 
        hr_y_imag = hr_fft.imag
        hr_y_real = hr_fft.real

        hr_fft_output = torch.complex(hr_y_real, hr_y_imag)

        loss_fft = self.loss_weight_fft * self.criterion(pred_fft_output, hr_fft_output)
